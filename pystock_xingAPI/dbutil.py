...
class DBUtil:
	...
	def create_table_for_outblock(cursor, table_name, tr_code, outblock_name, key_names = None, index_names = None, append=None):
		"""
		cursor : cursor of sqlite3
		...
		if append is not None:
			key=list(append.keys())[0]
			value=list(append.values())[0]
			outblock[key] = value
	...
	def _create_table_for_outblock(cursor, table_name, outblock_for_db, key_names = None, index_names = None):
		type_table = {'long':int, 'char':str, 'float':float, 'double':float, 'date':str}
